...
- ✅ Добавлен whitelist для ограничения доступа по Telegram ID
- ✅ Защита всех обработчиков сообщений (команды, текст, фото, callback)
...
- `GOOGLE_API_KEY` - ключ Google Custom Search API (опционально)
- `GOOGLE_CX` - ID поисковой системы Google (опционально)
- `ALLOWED_USER_IDS` - список разрешенных Telegram ID (через запятую)

#### Ограничение доступа (Whitelist)

Для приватного использования бота добавьте Telegram ID разрешенных пользователей:
...
2. Или посмотрите в логах бота при первом обращении
3. ID также отображается в сообщении "Доступ запрещен"

**Примечание:** Если `ALLOWED_USER_IDS` пустой или не указан, бот будет доступен всем пользователям.
